...
- **Non-Postgres / non-MSSQL stack** — the database-side generators and migration tooling are half the value. Postgres and MSSQL are both supported as of v0.3.1 (flip functions, desc generators, bulk backfill, autofill triggers all reach feature parity across vendors); other engines are not on the roadmap.
